...
struct noop * delNoop(noop ** tree, char *username)
{
    if (*tree==NULL) return NULL;
    else if (strcmp((*tree)->username, username)>0) return delNoop(&(*tree)->left, username);
    else if (strcmp((*tree)->username, username)<0) return delNoop(&(*tree)->right, username);
    else // T->key == x
    {
        noop *P = *tree;
        noop *del = (noop*)malloc(sizeof(noop));
        strcpy(del->username,(*tree)->username);
        strcpy(del->password,(*tree)->password);
        del->mark = (*tree)->mark;
        if ((*tree)->left == NULL) *tree = (*tree)->right;    // Node chi co cay con phai
        else if ((*tree)->right == NULL) *tree = (*tree)->left;   // Node chi co cay con trai
        else // Node co ca 2 con
        {
            noop *S = *tree, *Q = S->left;
...
int xoaUser(noop ** user)
{
    ...
    noop * del= delNoop(user, username);
